...
void main() {
  group('PreflightService', () {
    ...
    test(
      'deduplicates probes by URI and applies failures to each moment',
      () async {
        var readCalls = 0;
        var prepareCalls = 0;
        final service = PreflightService(
    ...
    test(
      'times out stalled probes without delaying another URI or save',
      () async {
        final records = _MemoryRecords();
    ...
    test(
      'times out stalled system status and still saves the record',
      () async {
        final records = _MemoryRecords();
        final service = PreflightService(
          canRead: (_) async => true,
